# Local source-availability helpers (docs/utils/PISPDocUtils.jl):
...
function local_material_state(profile)
    inspection = PISPDocUtils.inspect_edition(profile)
...
@testset "source availability helper fixtures" begin
    function fixture_roots(dir, edition; report=true, download=true)
        profiles = PISPDocUtils.source_availability_profiles(dir; env=Dict{String,String}())
    ...
    function populate_fixture(profile)
        for requirement in PISPDocUtils.edition_requirements(profile.edition)
    ...
    mktempdir() do dir
        profiles = PISPDocUtils.source_availability_profiles(dir; env=Dict{String,String}())
        @test all(profile -> PISPDocUtils.inspect_edition(profile).state == :absent, profiles)
        @test all(profile -> local_material_state(profile) == :skip, profiles)
    end

    mktempdir() do dir
        profile = fixture_roots(dir, "2024"; report=true, download=false)
        @test PISPDocUtils.inspect_edition(profile).state == :incomplete
    ...
    mktempdir() do dir
        ...
        summary = PISPDocUtils.source_availability_summary(profile)
    ...
    mktempdir() do dir
        ...
        requirement = first(filter(
            ...
            PISPDocUtils.edition_requirements("2026"),
        ))
    ...
    mktempdir() do dir
        ...
        profile = only(filter(
            p -> p.edition == "2024",
            PISPDocUtils.source_availability_profiles(dir; env=env),
        ))
    ...
    @test !isdefined(PISPDocUtils, :HTTP)
end

profiles = PISPDocUtils.source_availability_profiles(normpath(joinpath(@__DIR__, "..")))
