...
# Retirement tables identify expected closure years, while ParseISP applies additional package-defined scenario schedules when generating ISP 2024 datasets.
...
const RELIABILITY_2024 = ParseISP.source_spec(:existing_generator_reliability, 2024)
const RETIREMENTS_2024 = ParseISP.source_spec(:generator_retirements, 2024)
const WORKBOOK2024 = ParseISP.source_path(ISP2024.download_root, RELIABILITY_2024)
...
# ParseISP reads the existing and new-entrant blocks separately and maps them into generator and storage fields.

reliability_source_2024 = ParseISP.read_xlsx_rows(WORKBOOK2024, RELIABILITY_2024)
...
ParseISPDocUtils.markdown_table(reliability_2024)
#-
...
ParseISPDocUtils.markdown_table(long_duration_2026)
#-
...
ParseISPDocUtils.markdown_table(other_outages_2026)
#-
...
retirement_source_2024 = ParseISP.read_xlsx_rows(WORKBOOK2024, RETIREMENTS_2024)
retirement_2024 = DataFrame(
    retirement_source_2024[2:10, 1:3],
    Symbol.(["Station", "DUID", "Expected closure year"]);
    makeunique = true,
)
ParseISPDocUtils.markdown_table(retirement_2024)
#-
...
ParseISPDocUtils.markdown_table(retirement_2026)
#-
...
retirement_conventions = DataFrame([
    (
        ...
        scenario = ParseISP.ID2SCE[scenario_id],
        retirement_stations = length(ParseISP.Retirements2024[scenario_id]),
        capacity_reduction_stations = length(ParseISP.Reduction2024[scenario_id]),
    )
    for scenario_id in sort(collect(keys(ParseISP.ID2SCE)))
])
ParseISPDocUtils.markdown_table(retirement_conventions)
#-
