...
function low_output_events_for(tech, loc, hh_cols, threshold, yr)
    ...
    daily = ParseISPDocUtils.row_mean(summer, hh_cols)
...
for (season_type, year_list) in (("Hot Summers", HOT_SUMMERS), ("Cool Summers", COOL_SUMMERS))
    for yr in year_list
        ...
        vals = ParseISPDocUtils.row_mean(df[summer_mask, :], HH_COLS_SOL)
        push!(
            rows,
            (
...
ParseISPDocUtils.write_table(hot_cool_summer_solar_summary, SCRIPT_STEM, "hot_cool_summer_solar_summary")
ParseISPDocUtils.markdown_table(hot_cool_summer_solar_summary)
````
...
ParseISPDocUtils.write_table(low_output_events, SCRIPT_STEM, "low_output_events")
...
ParseISPDocUtils.markdown_table(low_output_event_summary)
````
...
for yr in 2011:2023
    ...
    any(summer_mask) || continue
    summer = df[summer_mask, :]
    daily = ParseISPDocUtils.row_mean(summer, HH_COLS_SOL)
...
ParseISPDocUtils.write_table(worst_solar_day_summary, SCRIPT_STEM, "worst_solar_day_summary")
ParseISPDocUtils.markdown_table(worst_solar_day_summary)
````
...
ParseISPDocUtils.write_table(worst_solar_day_profile, SCRIPT_STEM, "worst_solar_day_profile")
ParseISPDocUtils.markdown_table(worst_solar_day_profile)
````
...
ParseISPDocUtils.write_table(monthly_cf_2019_summary, SCRIPT_STEM, "monthly_cf_2019_summary")
ParseISPDocUtils.markdown_table(monthly_cf_2019_summary)
````
...
if df !== nothing
    ...
    summer = df[summer_mask, :]
    daily = ParseISPDocUtils.row_mean(summer, HH_COLS_SOL)
    rolling3 = ParseISPDocUtils.rolling_mean(daily, 3)
...
ParseISPDocUtils.write_table(black_summer_2019_daily_cf, SCRIPT_STEM, "black_summer_2019_daily_cf")
ParseISPDocUtils.markdown_table(first(black_summer_2019_daily_cf, 15))
````
...
savefig(p_hc, ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_hot_vs_cool_summer_solar.png"))
ParseISPDocUtils.embed_figure(ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_hot_vs_cool_summer_solar.png"), "04_hot_vs_cool_summer_solar.png")
````
...
p_low_out = plot(p_sol_hist, p_wind_hist, p_worst_days, p_worst_profile, layout=(2,2), size=(1400, 1000),
                  left_margin=5Plots.mm, bottom_margin=5Plots.mm)
savefig(p_low_out, ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_low_output_events.png"))
ParseISPDocUtils.embed_figure(ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_low_output_events.png"), "04_low_output_events.png")
````
...
````julia
monthly_table_path = ParseISPDocUtils.table_path(SCRIPT_STEM, "monthly_cf_2019_summary")
if isfile(monthly_table_path)
    ...
    p_monthly = bar(month_labels, monthly_df.mean_cf, yerror=monthly_df.std_cf, color=:darkorange, alpha=0.6,
                    ...
                    left_margin=5Plots.mm, bottom_margin=5Plots.mm)
    savefig(p_monthly, ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_monthly_cf_2019.png"))
    ParseISPDocUtils.embed_figure(ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_monthly_cf_2019.png"), "04_monthly_cf_2019.png")
end
````
...
if df_2019 !== nothing
    ...
    plot!(p_black, title="Solar $(SOLAR_LOC) — Summer 2019 (Black Summer)", ylabel="Daily Mean CF",
          ...
          left_margin=5Plots.mm, bottom_margin=5Plots.mm)
    savefig(p_black, ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_summer_2019_black_summer.png"))
    ParseISPDocUtils.embed_figure(ParseISPDocUtils.figure_path(SCRIPT_STEM, "04_summer_2019_black_summer.png"), "04_summer_2019_black_summer.png")
end
````
